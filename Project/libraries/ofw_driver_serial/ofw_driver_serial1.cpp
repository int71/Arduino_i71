...
#include				"ofw_driver_serial1.hpp"
...
VOID					SERIAL1::stDelete(VOID){
	return;
}

VOID					SERIAL1::stReset(VOID){
	st.biReadDone=0;
	st.biReadCurrent=0;
	return;
}

BYTE					SERIAL1::stbRead(VOID){
	BYTE					breturn;
...
VOID					SERIAL1::stPrintClear(VOID){
	stPrint(ARGSTR("\x1b[2J"));
	stPrintSetPosition(0,0);
	return;	
}

VOID					SERIAL1::stPrintSetPosition(
	BYTE					bix,
	BYTE					biy
){
	stPrint(ARGSTR("\x1b["));
...
VOID					SERIAL1::stPrintSetForeColor(
	BYTE					bccolor
){
...
VOID					SERIAL1::stPrintSetBackColor(
	BYTE					bccolor
){
