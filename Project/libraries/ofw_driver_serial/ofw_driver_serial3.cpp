...
**									ofw										**
**																			**
**	driver/serial3									2024 written by int71	**
 ****************************************************************************/
...
#include				"ofw_driver_serial3.hpp"
...
VOID					SERIAL3::stDelete(VOID){
	return;
}

VOID					SERIAL3::stReset(VOID){
	st.biReadDone=0;
	st.biReadCurrent=0;
	return;
}

BYTE					SERIAL3::stbRead(VOID){
	BYTE					breturn;
...
VOID					SERIAL3::stPrintClear(VOID){
	stPrint(ARGSTR("\x1b[2J"));
	stPrintSetPosition(0,0);
	return;	
}

VOID					SERIAL3::stPrintSetPosition(
	BYTE					bix,
	BYTE					biy
){
	stPrint(ARGSTR("\x1b["));
...
VOID					SERIAL3::stPrintSetForeColor(
	BYTE					bccolor
){
...
VOID					SERIAL3::stPrintSetBackColor(
	BYTE					bccolor
){
